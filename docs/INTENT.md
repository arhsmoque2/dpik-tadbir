...
7. **AI-Optional Review (Reduce Total Dependency on AI Chat)** ([`ADR-022`](adr/ADR-022-bundle-based-retrieval-ai-optional-review-and-adaptive-navigation.md)): Every retrieval executed against a filter is materialized as a **Bundle** — a durable, human-readable set of the matched emails, not merely ephemeral context fed to a prompt. An executive can open a Bundle, read the retrieved emails directly, and write their own free-text notes on it without ever invoking the AI. Copilot is one optional action available on top of a Bundle (summarize, draft, find), never the only way to use one. This principle governs every future feature: an AI capability is additive to a human-usable surface, not a replacement for it.
...
6. **Not an AI-Chat-First Product**: Retrieved mail (a Bundle) must be fully readable and annotatable by a human with zero AI involvement. A dashboard or list view that only exposes retrieved content through a chat prompt is a regression against [`INTENT-03`](#intent-03-core-operating-principles) item 7, not a valid implementation.
